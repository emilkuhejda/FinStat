...
{
    public class CashFlowPageViewModel : ViewModelBase
    {
        ...
        public async Task InitializeAsync(IEnumerable<IncomeStatement> incomeStatements, SearchResult searchResult, bool quarterlyData)
        {
            ...
            if (result.success)
            {
                ...
                Rows = gridGenerator.GenerateCashFlowStatements(searchResult.Name, result.payload, incomeStatements.ToList(), _applicationSettings.DisplayUnit);
                _isInitialized = true;
            }
            else
            {
